...
enum StatusNetwork {
    ...
    private static func request(_ url: URL, timeout: TimeInterval, headers: [String: String] = [:]) async throws -> Data {
        ...
        do {
            ...
            if error.code == .timedOut { throw NetworkError(message: "请求超时") }
            if error.code == .cancelled { throw NetworkError(message: "请求已取消") }
            throw NetworkError(message: "网络不可用")
    ...
    static func measureGateway() async -> GatewayStatus {
        ...
        if !successful.isEmpty {
            ...
            let median = successful.count.isMultiple(of: 2) ? (successful[middle - 1] + successful[middle]) / 2 : successful[middle]
            return GatewayStatus(latencyMS: median, measuredAt: Date(), responseStatus: 200, classification: .ok, detail: "ok · \(median) ms")
        }
        if let status = measurements.map(\.statusCode).first(where: { $0 > 0 }) {
            let classification: GatewayClassification = (300..<400).contains(status) ? .redirect : (400..<500).contains(status) ? .clientError : status >= 500 ? .serverError : .unavailable
            return GatewayStatus(latencyMS: nil, measuredAt: Date(), responseStatus: status, classification: classification, detail: "HTTP \(status)")
        }
    ...
    private static func measureGatewayOnce() async -> GatewayMeasurement {
        ...
        do {
            ...
            return GatewayMeasurement(statusCode: status, latencyMS: max(1, Int(Date().timeIntervalSince(started) * 1000)), timedOut: false)
        } catch let error as URLError {
    ...
    static func probeCustomMonitors(_ monitors: [CustomMonitor]) async -> [CustomMonitorResult] {
        let enabled = monitors.filter(\.enabled)
        return await withTaskGroup(of: CustomMonitorResult?.self, returning: [CustomMonitorResult].self) { group in
            for monitor in enabled { group.addTask { await probe(monitor) } }
