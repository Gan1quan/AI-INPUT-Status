...
/// Swift owns iOS-facing concerns; deterministic status calculations live in Rust.
enum RustCore {
    ...
    static func successRate(_ values: [Bool?]) -> Double? {
        guard !values.isEmpty else { return nil }
        let states = values.map { value -> Int8 in
            guard let value else { return -1 }
            return value ? 1 : 0
        }
        let result = states.withUnsafeBufferPointer { buffer in
            ai_input_success_rate(buffer.baseAddress, buffer.count)
        }
    ...
    static func chooseBackup(states: [Bool?], latencies: [Int?]) -> Int? {
        ...
        let stateValues = states.map { value -> Int8 in
            guard let value else { return -1 }
            return value ? 1 : 0
        }
